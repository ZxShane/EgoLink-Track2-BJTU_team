...
USER_API_BASE_URL = os.environ.get("USER_API_BASE_URL", os.environ.get("LLM_API_BASE_URL", "https://api.example.com/v1"))
...
def call_user_model(messages, max_retries=3, enable_thinking=None):
    ...
    for attempt in range(max_retries):
        try:
            client = _openai_client(USER_API_KEY, USER_API_BASE_URL)
            kwargs = {
