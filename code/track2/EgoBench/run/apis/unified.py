...
def _call_gemini_api(messages, video_url, max_retries, base_delay):
    ...
    client = _openai_client(GEMINI_API_KEY, os.environ.get("LLM_API_BASE_URL", "https://api.example.com/v1"))
    last_error = None
    for attempt in range(max_retries):
        try:
...
def _call_generic_api(messages, model_name, max_retries, base_delay, enable_thinking):
    ...
    base_url = os.environ.get("SERVICE_API_BASE_URL", SERVICE_API_BASE_URL).rstrip("/")
    if base_url.endswith("/chat/completions"):
        url = base_url
    else:
        url = f"{base_url}/chat/completions"
    config = MODEL_CONFIGS.get(model_name, {})
    token = config.get("token") or os.environ.get("SERVICE_API_KEY", SERVICE_API_KEY) or os.environ.get("API_KEY", "")
    ...
    for attempt in range(max_retries):
        try:
            response = _requests_post(url, headers=headers, json=payload)
